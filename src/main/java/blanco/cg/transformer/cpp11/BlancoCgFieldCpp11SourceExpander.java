 ...
 * @author IGA Tosiki
 */
class BlancoCgFieldCpp11SourceExpander {
    /**
    ...
    public void transformField(final BlancoCgField cgField,
        ...
        new BlancoCgLangDocCpp11SourceExpander().transformLangDoc(cgField
        ...
        if (cgField.getFinal()) {
            // フィールドへのfinal修飾は readonly と表現するように機能強化。
            // constではないところにマッピング。
            buf.append("readonly ");
        }

        // C++ においては、この方式では追加不能。import文に型を追加。
        // C++ においては、この方式では追加不能。argSourceFile.getImportList().add(cgField.getType().getName());

        // フィールド生成の本体部分を展開します。
        buf.append(BlancoCgTypeCpp11SourceExpander.toTypeString(cgField.getType())
                + " ");
